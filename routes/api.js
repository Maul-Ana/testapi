...
var creatorList = ['@MR','@MRGANZ','@riski.pfp','@MRGANTENG', '@mrganssss','@riskiganteng','@riskigantengg'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6288289337042 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------RiskiGanteng'+'MR--GANS';
        
 
 async function cekApiKey(api) {
 	ap = await riski.findOne({apikey:api})
 return ap;
 }
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'riskiganz') return res.json(loghandler.invalidKey)
